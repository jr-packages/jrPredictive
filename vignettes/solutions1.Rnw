...
\hlkwd{install.packages}\hlstd{(}\hlstr{"jrPredictive"}\hlstd{)}
\end{alltt}
\end{kframe}
\end{knitrout}
...
\definecolor{shadecolor}{rgb}{0.969, 0.969, 0.969}\color{fgcolor}\begin{kframe}
\begin{alltt}
\hlcom{# There definitely appears to be some trend in the residuals.}
\hlcom{# The curved shape indicates that we potentially require some transformation of variables.}
...
\definecolor{shadecolor}{rgb}{0.969, 0.969, 0.969}\color{fgcolor}\begin{kframe}
\begin{alltt}
\hlcom{#We seem to slightly over estimate more often than not in the 25-35 range.}
\hlcom{#For the upper end of the range we seem to always under estimate the true values.}
\end{alltt}
\end{kframe}
\end{knitrout}
...
\definecolor{shadecolor}{rgb}{0.969, 0.969, 0.969}\color{fgcolor}\begin{kframe}
\begin{alltt}
\hlcom{# We are struggling to justify the assumption of normality in the residuals here,}
\hlcom{# all of the diagnostics indicate patterns remain in the residuals that are currently}
\hlcom{# unexplained by the model.}
\end{alltt}
\end{kframe}
\end{knitrout}
\end{itemize}
...
\begin{alltt}
\hlcom{# We are struggling to justify the assumption of normality in the residuals here,}
\hlcom{# all of the diagnostics indicate patterns remain in the residuals that are currently unexplained by the model}
\end{alltt}
\end{kframe}
\end{knitrout}
...
\definecolor{shadecolor}{rgb}{0.969, 0.969, 0.969}\color{fgcolor}\begin{kframe}
\begin{alltt}
\hlcom{# The residual diagnostics indicate a better fit now that the quadratic term has been included.}
\end{alltt}
\end{kframe}
\end{knitrout}
\end{itemize}
...
\definecolor{shadecolor}{rgb}{0.969, 0.969, 0.969}\color{fgcolor}\begin{kframe}
\begin{alltt}
\hlcom{#Perhaps the residuals more closely match the assumption of normality under this transformation. However we need to be careful about interpretation now as the response is on the log scale. Likewise for prediction we need to remember to undo the transformation.}
\end{alltt}
\end{kframe}
\end{knitrout}
...
The \cc{jrPredictive} package contains a \cc{plot3d} function to help with viewing these surfaces in 3D as in figure~\ref{fig:fesurface}.\sidenote{We can also add the observed points to the plot using the \cc{points} argument to this function, see the help page for further information.}
\begin{knitrout}\small
\definecolor{shadecolor}{rgb}{0.969, 0.969, 0.969}\color{fgcolor}\begin{kframe}
\begin{alltt}
...
\definecolor{shadecolor}{rgb}{0.969, 0.969, 0.969}\color{fgcolor}\begin{kframe}
\begin{alltt}
\hlstd{m4} \hlkwb{=} \hlkwd{train}\hlstd{(FE}\hlopt{~}\hlstd{EngDispl}\hlopt{*}\hlstd{NumCyl} \hlopt{+} \hlkwd{I}\hlstd{(NumCyl}\hlopt{^}\hlnum{5}\hlstd{),}
           \hlkwc{data} \hlstd{= cars2010,} \hlkwc{method} \hlstd{=} \hlstr{"lm"}\hlstd{)}
\end{alltt}
\end{kframe}
\end{knitrout}
...
%' I have a set of data that you haven't yet seen. Once you are happy with your model you can validate it using the \cc{validate} function in the \cc{jrPredictive} package.
%'
%' <<eval = FALSE>>=
...
\begin{table}[!h]
  ...
  \midrule
  diamonds         & \cc{ggplot2} & price \\
  Wage             & \cc{ISLR}    & wage \\
  BostonHousing    & \cc{mlbench} & medv \\
  \bottomrule
\end{tabular}
\end{table}
