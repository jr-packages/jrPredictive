...
\definecolor{shadecolor}{rgb}{0.969, 0.969, 0.969}\color{fgcolor}\begin{kframe}
\begin{alltt}
\hlcom{# We now have a curved decision boundary.}
\hlcom{# There are two regions of where we would predict MM, bottom left, and a tiny one up in the top right.}
\end{alltt}
\end{kframe}
\end{knitrout}
...
\definecolor{shadecolor}{rgb}{0.969, 0.969, 0.969}\color{fgcolor}\begin{kframe}
\begin{alltt}
\hlcom{#The model is fairly good at picking up both positive events, person buys CH, and negative events, MM.}
\end{alltt}
\end{kframe}
\end{knitrout}
...
\definecolor{shadecolor}{rgb}{0.969, 0.969, 0.969}\color{fgcolor}\begin{kframe}
\begin{alltt}
\hlcom{#Accuracy increases at first with knn before then getting worse after a peak value of 9.}
...
\definecolor{shadecolor}{rgb}{0.969, 0.969, 0.969}\color{fgcolor}\begin{kframe}
\begin{alltt}
\hlcom{#The KNN regression model is not as good as the linear model at predicting the test set. It overestimates more at the lower end.}
\end{alltt}
\end{kframe}
\end{knitrout}
...
\end{verbatim}
\begin{alltt}
\hlcom{#The validation set approach was quickest, however we must bear in mind that the conclusion here}
\hlcom{#was different to the other cross validation approaches. The two k--fold cross validation estimates of RMSE and the bootstrap}
\hlcom{#estimates all agreed with each other lending more weight to their conclusions. Plus we saw in the lectures that validation set}
\hlcom{#approach was prone to highly variable estimates meaning we could get a different conclusion using a different hold out set.}
\hlcom{#Either of the two k--fold cross validation methods would be preferable here.}
\end{alltt}
\end{kframe}
\end{knitrout}
\end{itemize}
...
\hlstd{maxabsres} \hlkwb{=} \hlkwa{function}\hlstd{(}\hlkwc{data}\hlstd{,} \hlkwc{lev} \hlstd{=} \hlkwa{NULL}\hlstd{,} \hlkwc{model} \hlstd{=} \hlkwa{NULL}\hlstd{)\{}
    ...
    \hlkwd{return}\hlstd{(}\hlkwd{c}\hlstd{(}\hlstr{"Max"} \hlstd{= m))}
\hlstd{\}}
\hlcom{# Test with pls regression}
\hlstd{tccustom} \hlkwb{=} \hlkwd{trainControl}\hlstd{(}\hlkwc{method} \hlstd{=} \hlstr{"cv"}\hlstd{,}
                     \hlkwc{summaryFunction} \hlstd{= maxabsres)}
\hlstd{mPLScustom} \hlkwb{=} \hlkwd{train}\hlstd{(FE}\hlopt{~}\hlstd{.,} \hlkwc{data} \hlstd{= cars2010,}
                   \hlkwc{method} \hlstd{=} \hlstr{"pls"}\hlstd{,}
               \hlkwc{tuneGrid} \hlstd{=} \hlkwd{data.frame}\hlstd{(}\hlkwc{ncomp} \hlstd{=} \hlnum{1}\hlopt{:}\hlnum{6}\hlstd{),}
               \hlkwc{trControl} \hlstd{= tccustom,}
               \hlkwc{metric} \hlstd{=} \hlstr{"Max"}\hlstd{,} \hlkwc{maximize} \hlstd{=} \hlnum{FALSE}\hlstd{)}
\hlcom{# success}
\hlcom{# not to suggest this is a good choice of metric}
\end{alltt}
\end{kframe}
\end{knitrout}
